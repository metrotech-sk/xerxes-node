...
class XerxesSystem:
    ...
    def __init__(
        self, roots: List[XerxesRoot], sample_period: float, syncing=False
    ):
        self.roots = roots
        self._errors = 0
            ...
            for leaf in root.leaves:
                ...
                try:
                    ...
                    for key, value in process_values.items():
                        ...
                        log.debug(
                            f"Leaf: {leaf.address}, label: {key}, key: {value}, val: {pv}"
                        )

                except ChecksumError:
                    self._errors += 1
                    log.warning(
                        f"message from leaf {leaf.address} has invalid checksum"
                    )

                except MessageIncomplete:
                    self._errors += 1
                    log.warning(
                        f"message from leaf {leaf.address} is not complete."
                    )
    ...
    def _average_measurements(self, data: Dict) -> None:
        log.debug(f"Before averaging: {data}")
        for label, values in data.items():
            for process_value, value_array in values.items():
                try:
                    data[label][process_value] = sum(value_array) / len(
                        value_array
                    )
                except ZeroDivisionError:
                    # delete empty lists
                    data[label][process_value] = None
        log.debug(f"After averaging: {data}")
        return data
    ...
    def dump(self, directory: str) -> None:
        ...
        data = {"measurements": self._average_measurements(data)}

        add_timestamp(data)

        # dump to file for later processing
        tf = tempfile.NamedTemporaryFile(
            delete=False, suffix=".dat", prefix="meas", dir=directory
        )
        pickle.dump(obj=data, file=tf)
        tf.close()
